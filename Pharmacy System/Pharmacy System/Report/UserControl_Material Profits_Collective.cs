...
{
    public partial class UserControl_Material_Profits_Collective : UserControl
    {
        SqlConnection sqlConnection = new SqlConnection(Class_Connection.con);
        SqlDataAdapter sqlDataAdapter;
        DataTable dataTable;

        private static UserControl_Material_Profits_Collective _instance;
        public static UserControl_Material_Profits_Collective Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new UserControl_Material_Profits_Collective();
                return _instance;
            }
        }
        public void Clear()
        ...
        public UserControl_Material_Profits_Collective()
        {
            InitializeComponent();
        }

        private void UserControl_Material_Profits_Collective_Load(object sender, EventArgs e)
        {
            Get_ItemName();
            Load_Data();
        }
        ...
        private string Check_Text()
        {
            string where = "where ";
            byte AddAnd = 0;
            if (ItemName_comboBox.Text == "" && !Buy_checkBox.Checked  && !Seles_checkBox.Checked && !ReturnBuy_checkBox.Checked 
        ...
        public void CompoBox_CheckedChanged(object sender, EventArgs e)
        {
            Serch();
        }
